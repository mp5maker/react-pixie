...
export const Zombie = ({ colors, width, getScore }: any) => {
  ...
  const [
    zombieObstacleMaterials,
    setZombieObstacleMaterials,
  ]: any = React.useState([]);
  ...
  useFrame((state, delta) => {
    if (zombieIdleMaterials.length > 0) {
      ...
      if (state.clock.elapsedTime - startTime > 0.05 && zombie && obstacle) {
        ...
        if (!isPlaying) {
          if (!isDead)
            nextSprite({ object: zombie, materials: zombieIdleMaterials });
        } else {
          ...
          if (isCollision) {
            ...
            if (zombie.position.x - 3 > obstacle.position.x) {
              nextSprite({
                object: obstacle,
                materials: zombieObstacleMaterials,
              });
  ...
  React.useEffect(() => {
    if (obstacle && zombie && !obstacle.collision) {
      obstacle.collision = () => {
        const collisionX =
          zombie.position.x + 0.5 >= obstacle.position.x &&
          zombie.position.x <= obstacle.position.x + 0.5;
    ...
    <>
      {zombieIdleMaterials.length > 0 && (
        <sprite
          ...
          args={[zombieIdleMaterials[0]]}></sprite>
      )}
      <line>
      ...
      {zombieObstacleMaterials.length > 0 && (
        <sprite
          ...
          args={[zombieObstacleMaterials[0]]}></sprite>
      )}
    </>
  );
};
