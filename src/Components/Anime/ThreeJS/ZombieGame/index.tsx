...
export const ZombieGame = ({ colors, theme, history }: any) => {
  ...
  const joystickMemo = React.useMemo(() => {
    return (
      <div className="zombie-game-controller">
        <Joystick width="175" height="175" theme={theme} colors={colors} />
      </div>
    );
  }, [theme]);

  const zombieMemo = React.useMemo(() => {
    return <Zombie getScore={getScore} width={width} colors={COLORS} />;
  }, [width, theme]);
