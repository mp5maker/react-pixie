...
export const AudioPlayer = ({}: any) => {
  ...
  const { setMusicSettings, ...otherSettingsProps }: any = React.useContext(
    MusicContext,
  );
  const [playAudio, setPlayAudio] = React.useState(AUDIO_NOT_PLAYING);
  const [loaded, setLoaded]: any = React.useState(0);
  const isSmallerTouchDevice = useMedia({
    query: `(pointer: coarse) and (max-width: 767px)`,
  });
  ...
  const setIsPlayingForSmallerDevice = React.useCallback(
    isPlaying => {
      setMusicSettings({
        ...otherSettingsProps,
        isPlaying,
