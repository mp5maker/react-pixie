...
export const Error = ({ history, location, match }: any) => {
  ...
  const { isLoading, error, data: response } = useQuery(
    ['posts', { params: { _limit: 20, _page: 3 } }],
    Query,
  );
  const { t } = useTranslation();
  useDocument({
    options: [
      { selector: 'title', value: `Photon's Portfolio: Page Do Not Exist` },
    ],
  });
  useSmoothScroll({});
          ...
          return (
            <React.Fragment>
              <div>
                ...
                <hr />
                <div className={`space-more`}>{designation}</div>
                <div>{department}</div>
                <div>{joining_date.substring(0, 10)}</div>
              </div>
            </React.Fragment>
          );
        }}
    ...
    return (
      <div className={`pdf-viewer`}>
        <Drawer
          ...
          allowSortUp={false}
          buttonShape={`round`}
          buttonDisplay={<FontAwesomeIcon icon={faFilePdf} />}>
          {({ toggleDrawer }: any) => {
            return (
              <PdfViewer
                list={data}
                ...
                body={({ row, column }: any) => {
                  ...
                  if (column == 'designation')
                    return get(row, 'designation', t(`HYPHEN`));
                  if (column == 'joining_date')
                    return get(row, 'joining_date', t(`HYPHEN`)).slice(0, 10);
                  if (column == 'name')
                    return get(row, 'department', t(`HYPHEN`));
                  return row[column];
                }}
                width={`100%`}
                height={`500px`}
  ...
  const zombieGameMemo = React.useMemo(() => {
    return <ZombieGame history={history} theme={theme} colors={Colors} />;
  }, [history, theme]);
    ...
    <motion.div
      ...
      <div className="row">
        <div className="col p-0">
          <AnimeThreeJSError history={history} theme={theme} colors={Colors} />
          <ErrorMessage />
        </div>
      </div>
      <div className="row">
        <div className="col p-0">
          {isLoading ? (
            <React.Fragment>{memoLoading}</React.Fragment>
          ) : (
            <React.Fragment>
              {memoCards}
              {memoPDF}
      ...
      <div className="row">
        <div className="col p-0">{zombieGameMemo}</div>
      </div>
    </motion.div>
  );
};
