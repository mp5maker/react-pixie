...
export class IndexedDB {
  ...
  public open() {
    const openPromise = new Promise((resolve, reject) => {
      this.request = this.indexedDB.open(
        this.databaseName,
    ...
    const prepareTablePromise = new Promise(resolve => {
      ...
      tables.forEach(tableName => {
        const objectStore = this.db.createObjectStore(tableName, {
          ...(this.tables[tableName].keyPath
            ? { keyPath: this.tables[tableName].keyPath }
            : {}),
          ...(this.tables[tableName].autoIncrement
            ? { autoIncrement: this.tables[tableName].autoIncrement }
            : {}),
        });
        ...
        columns.forEach((columnName: any) => {
          objectStore.createIndex(columnName, columnName, {
            ...(this.tables[tableName].columns[columnName].unique
              ? { unique: true }
              : { unique: false }),
          });
        });
